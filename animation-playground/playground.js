...
const clearPropsNoOpacity =
    'clip,clipPath,webkitClipPath,willChange,transform,transformOrigin'
...
animationExecute.addEventListener('click', () => {
    ...
    const {duration: durationIn, delay: delayIn, ...paramsIn} = Object.fromEntries(
    ...
    const {duration: durationOut, delay: delayOut, ...paramsOut} = Object.fromEntries(
    ...
    Array.from(animationSelectionForm.elements).forEach(el =>
...
onLoad(() => {
    ...
    if (window.tweenEngine2) {
        ...
        animations = window['santa-animations'].create(
            window.tweenEngine3.create(window.gsap),
            window,
            viewMode()
        )
    }
    ...
    requestAnimationFrame(() => Array.from(animationSelectionForm.elements).forEach(element => {
