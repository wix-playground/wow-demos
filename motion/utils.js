const CLIP_PARAMS = {
    ...
    top: (top, bottom, left, right, minimum) =>
        `${left}% ${top}%, ${right}% ${top}%, ${right}% ${top + minimum}%, ${left}% ${top + minimum}%`,
    right: (top, bottom, left, right, minimum) =>
        `${right - minimum}% ${top}%, ${right}% ${top}%, ${right}% ${bottom}%, ${right - minimum}% ${bottom}%`,
    center: (top, bottom, left, right, minimum, centerX, centerY) =>
    ...
    bottom: (top, bottom, left, right, minimum) =>
        `${left}% ${bottom - minimum}%, ${right}% ${bottom - minimum}%, ${right}% ${bottom}%, ${left}% ${bottom}%`,
    left: (top, bottom, left, right, minimum) =>
...
export function getTranslations(rect, originDirection, angle, scale = 1) {
    ...
    const x = originDirection.dy * height * Math.sin(-angleInRad) + originDirection.dx * width * Math.cos(angleInRad);
    const y = originDirection.dy * height * Math.cos(-angleInRad) + originDirection.dx * width * Math.sin(angleInRad);
...
export function getClipPolygonParamsExtended(
    ...
    const right = (contentRect.width / compRect.width) * 100 + left - (1 - scaleX) * 100;
    const bottom = (contentRect.height / compRect.height) * 100 + top - (1 - scaleY) * 100;
...
export function getAdjustedDirection(paramsMap, direction, angleInDeg) {
    ...
    const newIndex = (index + (directions.length - 1) * shiftBy) % directions.length;
